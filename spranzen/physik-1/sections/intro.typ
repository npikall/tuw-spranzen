...
= Mechanik
== Kinematik
#question()[Nennen Sie die 7 Grundgroessen der Physik.]
- Laenge
- Zeit
- Masse
- Stromstaerke
...
#question()[Nennen sie die Grundgroessen der Mechanik]
- Laenge
- Zeit
- Masse

#question()[Geben sie 3 Beispiele skalarer und 3 Beispiele vektorieller physikalischer Groessen an.]
- *skalar*: Masse, Zeit, // TODO: 3 Option?
- *vektoriell*: Geschwindigkeit, Kraft, Impuls

#question()[Rechnen Sie die kartesischen Koordinaten (x,y) des Punktes P in Polarkoordinaten (r, $phi$)
  um.]
$
  r = sqrt(x^2 + y^2)\
  phi = arctan(y / x)
...
#question()[Rechnen Sie die kartesischen Koordinaten (x,y, z) des Punktes P in Zylinderkoordinaten (r,
...
#question()[Was sind die Basis-(Einheits-)vektoren im kartesischen, polaren, zylindrischen und
...
#question()[Stellen sie die Volumenelemente der kartesischen, polaren und Kugelkoordinatensysteme dar.
  (Formel und Skizze)]
...
#question[Erklaeren sie das modell des Massenpunktes. Inwieweit ist das Modell relevant?]
...
#question[Fuer einen Massenpunkt ist die Beschleunigung $arrow(g) = "const"$ gegeben. Berechnen sie
  seine Bahnkurve. Welche Infromation ist noch noetig, um die Bewegung des Massenpunktes explizit zu
  beschreiben?]
#missing[]


#question[Geben sie die Bahnkurve fuer den schiefen Wurf an. Wie wird sie hergeleitet?]
$ arrow(r)(t) = 1/2 arrow(g) t^2 + arrow(v_0)t $
*Herleitung*: $arrow(r_0)$ entspricht dem Koordinatenursprung $arrow(r_0)=0$

#question[Ein Koerper wird unter einem Winkel $alpha$ schraeg nach oben geworfen. Der Betrag der
...
#question[Definition und Definition der Zentripetalbeschleunigung. Wo tritt sie auf?]
#missing[]

#question[Geben sie die Formel fuer die Orts-, Geschwindigkeits- und Beschleunigungsvektoren einer
...
#question[Was versteht man unter Winkelgeschwindigkeitsvektor?]
#missing[]

#question[Welche Kraefte muessen herrschen, damit sich ein Koerper entlang einer Kreisbahn bewegt?]
- Zentripetalkraft
#missing[formeln und kurze erklaerung]

#question[zeigen sie, dass bei gleichfoermiger rotation die zentripetalbeschleunigung zum zentrum und
  senkrecht zum Geschwindigkeitsvektor gerichtet ist.]

#question[Wie lauten die Newtonschen Axiome? Geben Sie Formeln an und erklären Sie die Bedeutung.]
...
#question[Wie sind schwere und träge Masse definiert?]
#missing[]
#question[Wie begründet Einstein seine Annahme, dass es keinen Unterschied gibt?]
#missing[]
#question[Wie kann man das Verhältnis schwerer zu träger Masse eyperimentell bestimmen?]
#missing[]
#question[Was ist ein Kraftfeld?]
#missing[]
#question[Was ist ein konservatives Kraftfeld?]
#missing[]
#question[Beispiel für konservative und nicht-konservative Kräfte.]
#missing[]
#question[Beweisen Sie dass das schwerefeld konservativ ist.]
#missing[]

== Dynamik
=== Reale und Scheinkräfte
#question[Erläutern sie den Begriff Scheinkraft.]
...
#question[Warum werden Scheinkräfte eingeführt?]
...
#question[Geben sie Beispiele für Scheinkräfte an.]
- Zentrifugalkraft
- Corioliskraft

#question[Begründen Sie das Auftreten von Scheinkräften in beschleunigten Bezugsystemen (an Beispielen
...
#question[Galileische Transformation]
...
#question[Zeigen Sie dass die Kraft bei einer Galileitransformation nicht verändert wird.]
...
#question[Welche der Größen sind in inertialen Bezugsystemen invariant? (r,v,*a,F*)]
...
#question[Wie ist Arbeit allgemein definiert?]
$
...
#question[Welche Eigenschaften hat diese Größe (skalare oder vektorielle, Wegabhängigkeit, Vorzeichen)]
...
#question[Definition der potentiellen Energie (allgemein) und der kinetische Energie.]
...
#question[Wie lauten die Maßeinheiten der Arbeit, der kinetischen, der potentiellen Energien?]
- Arbeit: Joule
- Pot. Energie: Joule
- Kin. Energie: Joule

#question[Konservative Kräfte]
...
#question[Erklären sie warum die definition der potentiellen energie nur dann sinn hat, wenn das
  kraftfeld konservativ ist?]
$E_"pot"$ hängt nur vom Ort von P ab, nicht vom Weg.

#question[Beweisen Sie den energieerhaltungssatz der mechanik.]
$
...
#question[berechnen sie die arbeit die aufzubringen ist um einen block der masse m um die höhe h zu
  heben.]
$
  E_"pot" (h=0) & = 0 \
  E_"pot" (h) & = - integral_0^h (m dot arrow(g)) dif arrow(z) = - integral_0^h m dot (-g) dif z = m dot g dot h
$
#question[in welchen zusammenhang stehen die kraft und die potentielle energie?]
- 1D: $(dif E_"pot")/(dif x) = -F_x$
- 3D: $F = - gradient E_"pot"$

#question[wie lautet der energieerhaltungssatz der mechanik?]
$
  W_"ges" = W_"kin" + W_"pot" = "const"
$


#question[unter welchen bedingungen gilt der energieerhaltungssatz der mechanik?]
Newtonsches Axiom für konservative Kräfte

#question[welche reibungskräft kennen sie?]
- haftreibung
- gleitreibung
- rollreibung
- stoke'sche reibung

#question[was ist die ursache der reibung zwischen 2 festen kontaktflächen?]
...
#question[was unterscheidet haft- von gleitreibung, und welche ist größer?]
Verzahnung bei Haftreibung ist größer, Gleitreibung trägt nur Spitzen ab.

$
  "Haftreibung" > "Gleitreibung"
$
#question[Berechnen sie die kraft, die notwendig ist um einen körper der masse m in horizontaler
...
#question[Wie lautet die formel für den schwerpunkt eines systems aus mehreren massenpunkten?]
$
  arrow(r_s) = (sum_i^n m_i arrow(r_i))/(sum_i^n m_i)
$
#question[wie definiert man impuls? in welchem zusammenhang stehene die auf den körper wirkende kraft
  und sein impuls?]
/ Impuls: $arrow(p) = m dot arrow(v)$

Zeitliche Ablietung des Impulses ist gleich der Wirkenden Kraft.

#question[Wie lautet der Schwerpunktsatz?]
...
#question[Wie leitet sich der Schwerpunktsatz her?]
- Impulserhaltung: ...
- Def. Schwerpunkt: ...
- Zeitableitung: ...
...
#question[Wie lautet der Impulserhaltungsatz?]
...
#question[Wie leitet sich der Impulserhaltungsatz her?]
...
#missing[formeln]
#question[Wende den Impulserhaltungssatz auf einen elastischen Stoss einer Kugel gegen eine Wand an.]
#missing[]
#question[Was passiert bei einem inelastischen Stoss?]
#missing[]
#question[Unter welchem Winkel muesste der Koerper auftreffen, damit der Impulsuebertrag maximal ist?
  Begruende die Antwort.]
In einem winkel von 0° frontal
#missing[formeln]

#question[Zwei reibungsfrei auf einer horizontalen Ebene entlang einer gemeinsamen Geraden rollenden
  ...
  elastisch ist?]
#missing[]
#question[Beschreiben sie die Funktionsweise des Raketenantriebs.]
...
#question[Erklären sie den Rückstoss einer Schusswaffe.]
...
$
#question[Eine Kugel trifft unter einem Winkel auf eine Wand. Der Stoss ist elastisch. Welchen Impuls
  und welche Richtung besitzt die Kugel nach dem Stoss? Formeln und Skizze.]
#missing[]
#question[Welche Erhaltungssaetze muessen bei welchen Stoessen gelten?]
- Elastischer Stoss:
  - Impulserhaltung
