...
#question[Wie lautet die Schwingungsgleichung der gedämpften Schwingung? Wie wird sie aufgestellt und
...
#question[Geben Sie einen Lösungsansatz an und zeigen Sie, dass er die Schwingungsgleichung erfüllt]
#missing[]
#question[Diskutieren Sie die verschiedenen Lösungen die sich ergeben und skizzieren Sie die
  Bewegungsabläufe]
#missing[]
#question[Betrachten Sie den Fall einer horizontal schwingenden Masse an einer Feder die durch die Luft
  gedämpft wird. Wie lautet die Schwingungsgleichung (Erläutere die einzelnen Terme)?]
$
  m x^4 + b x' + k x = 0
$

#question[Diskutieren Sie die verschiedenen Lösungen und zeichnen Sie die Weg-Zeit Diagramme der
  Bewegungsabläufe]
#missing[]
#question[Was ist das logarithmische Dekrement?]
...
#question[Lösen Sie die Bewegungsgleichung eines eindimensionalen ungedämpften Masse/Feder Systems]
$
...
#question[Welche Bedingungen muss die rückwirkende Kraft erfüllen?]
$
...
#question[Wie groß ist die Eigenfrequenz?]
$
  omega_0 = sqrt(c_F/m) --> "Kreisfreq. ist immer positiv"
$

#question[Skizzieren sie den zeitlichen Verlauf vom potentieller und kinetischer Energie]
#missing-plot()
#question[Wie lautet die Schwinungsgleichung für Masse/Feder System und für mathematisches Pendel]
- Masse/Federsystem: $x(t) = c dot sin(omega_0 dot t + psi)$
- Mathematisches Pendel: $phi(t) = A dot sin(omega_0 dot t + psi)$

#question[Welche der Eigenfrequenzen hängt von der Masse ab, welche nicht?]
Die Feder haengt von der Masse ab.
Das Pendel nicht.

#question[Sie bringen beide Systeme auf den mind. Welche eigenfrequenz ändert sich gegenüber der auf
...
#question[Gegeben sei ein Masse/Feder-System auf einer reibungsfreien Unterlage. Leiten Sie die
  Bewegungsgleichung der ungedämpften Schwingung her]
#missing[]
#question[Skizzieren Sie den Bewegungsablauf der Masse in einem Weg-Zeit-Diagramm, und zeichnen Sie
...
#question[Wie groß ist die Eigenfrequenz und die Periodendauer der Schwingung?]
$
  omega_0 = sqrt(c_F/m) = (2 pi)/T ==> T = omega_0/(2 pi)
$

#question[Wie verändert sich die Eigenfrequenz der harmonischen Schwingung, wenn die Masse verdoppelt,
  die Federkonstante jedoch konstant gehalten wird?]
wird um $sqrt(2)$ weniger ($omega_0 = sqrt(c_F/m)$)

#question[Wie lauten die zeitabhängigen Geschwindigkeit v(t) und die Beschleunigung a(t) und wie groß
...
#question[Gegeben ist ein mathematisches Pendel. Stellen Sie seine Bewegungsgleichung mit Hilfe des
...
#question[Was ist die Rückübersetzung Kraft des Pendels?]
Schwerkraft: $m dot g$

#question[Wie groß ist die eigenfrequenz des Pendels ?]
$
  omega_0 = sqrt(g/l)
$

#question[Warum hängt die Eigenfrequenz nicht von der Masse des Pendels ab?]
Weil die Energien beim Energieerhaltungssatz gleichgesetzt und gekuerzt werden

== Erzwungene Schwingung
#question[Wie lautet die Schwingungsgleichung?]
$
  x(t) = A(Omega) dot cos(Omega dot t - psi)
$
Schwingung, Kreisfrequenz = Erregerfrequenz $Omega$

#question[Wie wird sie aufgestellt und was bedeuten die einzelnen Terme?]
#missing[]
#question[Mit welcher Frequenz schwingt das System im stationären Zustand?]
Stationaere erzwungene Schwingung: Frequenz unabhaengig von Eigenfrequenz (nur durch Aeussere Anregung $Omega$)

#question[Wie hängen Amplitude und Phasenwinkel von der Frequenz ab?]
#missing[]
#question[Was ist die Resonanzkatastrophe und wie kann sie vermieden werden?]
...
#question[Skizzieren Sie Amplitude und Phasenwinkel im stationären in Abhängigkeit von der Dämpfung und
  von $Omega$?]
#missing[]
== Einschwingvorgang
#question[Ein gedämpftes Masse/Feder System wird von einem Erreger, der sich periodisch mit der
...
$tau = 1 / beta$
#missing-plot()
#question[Was sind Schwebungen und warum sind sie beim Einschwingvorgang wichtig, wenn das
