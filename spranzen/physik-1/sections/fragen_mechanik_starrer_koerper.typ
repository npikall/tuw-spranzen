...
#question[Wie lautet die allgemeine Formel für den Drehimpuls?]
$
...
#question[Wie lautet der Betrag des Drehimpulses in Richtung der Drehachse?]
$
  L_(arrow(e_omega)) = omega Theta_(arrow(e_omega))
$
#question[Wie lauten die Gleichgewichtsbedingungen (bei Angriff äußerer Kräfte $F_i$)?]
$
  sum_i arrow(F_i) (a) = 0\
  sum_i arrow(D)_(M_i)^(a) = sum_i (arrow(r_i) times arrow(F_i^a)) = 0
$

#question[Wie lautet das Hebelgesetz?]
$
  F_1 dot r_1 = F_2 dot r_2
$
Es sind nur aeussere Kraefte massgebend. Die Kraft auf das Auflager leistet keine Arbeit.

#question[Welche Methode verwendet man bei der Herleitung des Hebelgesetzes?]
- Prinzip der virtuellen Verschiebung
- Prinzip der virtuellen Arbeit

#question[Wann befindet sich ein starrer Körper im Gleichgewicht?]
$
  sum_i arrow(F_i)^a = 0\
  sum_i arrow(D)_(M_i)^(a) = sum_i (arrow(r_i) times arrow(F_i^a)) = 0
$

#question[Zeigen Sie für 3 in einer Ebene liegende Kräfte, die Gleichgewichtsbedingungen]
$
  sum_i^3 arrow(F_i) = 0\
  arrow(F_1) + arrow(F_2) + arrow(F_3) = 0
$

#question[Wie findet man die Gleichgewichtsbedingungen, wenn 2 der 3 Kräfte parallel sind?]
...
=== Rotationsbewegung
#question[Sie lassen einen Hohl- und einen Vollzylinder gleichen Außendurchmessers ($R_a$) und gleicher
...
#question[Welche Größen der Rotation entsprechen den translatorischen Größen Impuls, Kraft und
...
#question[Wie lautet der Energieerhaltungssatz der Mechanik bei Berücksichtigung der Rotation?]
...
#question[Wie lautet der Ausdruck für die kinetische Energie bei Rotation um eine starre Achse?]
Rotationsenergie:
$
  W_"kin"^"rot" = 1/2 omega L arrow(e_omega) = 1/2 omega Theta arrow(e_omega)
$

#question[Geben Sie die Gesamtenergie an, wenn der Rotation eine Translationsbewegung überlagert ist.
  Welcher Energiebetrag ist eine Erhaltungsgröße?]
#missing[]
#question[Skizzieren Sie die Berechnung des Trägheitsmomentes für einen Hohlzylinder beiRotation um die
  Zylinderachse]
#missing[]
#question[Wie ist das Trägheitsmoment definiert?]
Mass fuer den Widerstand den ein Koerper der Veraenderung seiner Winkelgeschwindigkeit entgegensetzt.
#missing[]

#question[Wie hängt das Trägheitsmoment mit dem Drehimpuls bei Rotation um eine ortsfeste Achse zusammen?]
#missing[]

#question[Zwei Kugeln (Radius R) sind durch eine masselose Stange (Länge l) miteinanderverbunden. a)
  ...
  R^2]
#missing[]
#question[Wie ist ein Massenelement, der Schwerpunkt und die Winkelgeschwindigkeit bei Rotation um eine
  ortsfeste Achse definiert (Skizze)?]
#missing[]
#question[Zeigen Sie wie man mit Hilfe des Prinzips virtueller Verschiebungen das Hebelgesetz ableiten
  kann]
