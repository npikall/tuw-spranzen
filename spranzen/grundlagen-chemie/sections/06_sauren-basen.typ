...
<säuren-und-basen>
== pH-Wert
_pH_ steht als Abkürzung für _potentia hydrogenii_ oder _pondus hydrogenii_. Der
pH-Wert ist ein Maß für die _Acidität_ bzw. _Basizität_ einer verdünnten
wässrigen Lösung.
...
=== Starke Säuren
Diese sind in Wasser vollständig dissoziiert, d.h. $alpha = 1$. Der pH-Wert der
Lösung ist somit gleich dem negativen dekadischen Logarithmus des Zahlenwerts
der Ausgangskonzentration der Säure HA:

#align(center, ce("c(H+) = c_0 (HA) => pH = -log(c_0 (HA))"))

#gc.example[
  #qty(10, "g") #ce("HCl") werden in #qty(800, "mL") Wasser gelöst. Welchen
  pH-Wert hat die entstandene Salzsäure?
...
=== Starke Basen
Diese sind in Wasser vollständig dissoziiert, d.h. $alpha = 1$. Bei den
(salzartigen) Hydroxiden der Alkalimetalle (MOH) liegt das #ce("OH−") Ion
bereits als Gitterbaustein vor. Die Konzentration der #ce("OH−") Ionen stimmt
also mit der aus der gelösten Stoffmenge berechneten Ausgangskonzentration $c_0$
(MOH) überein:

#align(center, ce("c(OH−) = c_0 (MOH) => pOH = -log(c_0 (HA))"))

Der zugehörige pH-Wert ergibt sich über das Ionenprodukt des Wassers mithilfe
des $"pK"_W$-Werts:
