...
#let linear-points(
  ...
  let xs = range(n).map(it => (
    x-range.at(0) + it * (x-range.at(1) - x-range.at(0)) / (n - 1)
  ))
  ...
  let ys = xs
    .enumerate()
    .map(it => slope * it.at(1) + noise.at(it.at(0)) + loc.at(1))
...
#figure(
  cetz.canvas({
    ...
    let plots = (
      (
        random-points(n: 30, loc: (0.5, 0.5), scale: (0.13, 0.13), seed: 42),
        red,
        $S_(X Y) approx 0$,
      ),
      (
    ...
    for p in plots {
      ...
      content(
        (v => cetz.vector.add(v, (0, 0.2)), "plot.north"),
        p.at(2),
        anchor: "south",
      )
      set-origin((5, 0))
